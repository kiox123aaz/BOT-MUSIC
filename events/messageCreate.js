...
client.on("messageCreate", async (message) => {
  ...
  if (nprefix.includes(client.user.id)) {
    message.reply(`**หากต้องการดูคำสั่งทั้งหมดของหนู ประเภท **\`/help\``);
  }
      ...
      return message.reply({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `** ❌ เธอไม่มี ${command.userPermissions} เพื่อเรียกใช้คำสั่ง.. **`
            ),
        ],
      });
      ...
      return message.reply({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `** ❌ หนูไม่มี ${command.botPermissions} เพื่อเรียกใช้คำสั่ง. **`
            ),
        ],
      });
